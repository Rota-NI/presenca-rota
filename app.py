...
# ==========================================================
# PDF CORRIGIDO (Removido caracteres Unicode)
# ==========================================================
class PDFRelatorio(FPDF):
    ...
    def footer(self):
        ...
        # Substituído caractere especial por hifen simples
...
try:
    ...
    if st.session_state.usuario_logado is None and not st.session_state.is_admin:
        ...
        with t3:
            ...
            st.info("Regras de Horário e Instruções de Instalação.")
        ...
        if ja and pos <= 3 and janela_conf:
            ...
            if st.session_state.conf_ativa:
                # Criando um container para evitar o retorno 'None' na interface
                with st.container():
        ...
        if dados_p_filtrado and len(dados_p_filtrado) > 1:
            insc = len(df_o)
            st.subheader(f"Inscritos: {insc} | Vagas: 38")
            ...
            st.download_button("📄 PDF", gerar_pdf_apresentado(df_o, resumo), "lista.pdf")
