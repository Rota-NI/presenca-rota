...
# --- CONFIGURAÇÃO DE ACESSO COM CACHE ---
    ...
    if len(dados_p) > 1:
        try:
            ...
            if ultima_assinatura_dt < marco_ciclo_atual:
                ...
                # Limpa também o checklist salvo ao virar o ciclo
...
try:
    ...
    if st.session_state.usuario_logado is None:
        ...
        if aberto:
            ...
            else: st.warning(f"✅ Presença registrada. Sua posição atual: {posicao_usuario}º")

        # --- FUNCIONALIDADE: LISTA DE PRESENÇA COM PERSISTÊNCIA ---
        if ja and posicao_usuario <= 2:
            ...
            if st.session_state.conferencia_ativa:
                st.info("Os nomes marcados ficarão salvos durante este ciclo.")
                
                # Criamos um container para os checkboxes
                with st.container():
                    for index, row in df.iterrows():
                        # Chave única para persistência baseada no nome
                        key_persistencia = f"presenca_{row['NOME']}"
                        
                        # Se não existir no session_state, inicializa como falso
                        if key_persistencia not in st.session_state:
                            st.session_state[key_persistencia] = False
                        
                        # Checkbox que atualiza o session_state automaticamente
                        st.checkbox(
                            f"{row['Nº']} - {row['GRADUAÇÃO']} {row['NOME']}", 
                            key=key_persistencia
                        )
        ...
        if len(dados_p) > 1:
            ...
            with col_pdf:
                ...
                for _, r in df.iterrows():
                    ...
                    pdf.ln()
                st.download_button("📄 BAIXAR PDF", pdf.output(dest="S").encode("latin-1"), "lista.pdf", "application/pdf", use_container_width=True)
            ...
            with col_wpp:
                ...
                st.markdown(f'<a href="{link_wpp}" target="_blank"><button style="width:100%; height:38px; background-color:#25D366; color:white; border:none; border-radius:4px; font-weight:bold; width:100%;">🟢 ENVIAR WHATSAPP</button></a>', unsafe_allow_html=True)
    ...
    if "429" in str(e): st.error("⚠️ Limite do Google atingido. Aguarde 10s.")
