...
def aplicar_ordenacao_e_numeracao(df):
    # 1. Padroniza o nome da coluna de destino para evitar erros de leitura
    if 'QG_RMCF_OUT' in df.columns:
        df = df.rename(columns={'QG_RMCF_OUT': 'QG_RMCF_OUTROS'})
    
    # 2. Define os pesos de prioridade
    ...
    # 3. Cria colunas auxiliares de ordenação
...
try:
    ...
    if st.session_state.usuario_logado is None:
        ...
        if len(dados_p) > 1:
            ...
            # SOLUÇÃO PARA AS DUAS COLUNAS: 
            # O st.dataframe com hide_index=True remove a coluna 0, 1, 2...
